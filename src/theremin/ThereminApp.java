...
public class ThereminApp extends JPanel implements ItemListener, KeyListener, MouseMotionListener//, JXInputAxisEventListener
...
protected static ImageIcon createImageIcon(String path,
                                           String description) {
    java.net.URL imgURL = ThereminApp.class.getResource(path);
    if (imgURL != null) {
...
public static void main(String args[])
{
    ...
    JComponent newContentPane = new ThereminApp();
...
class RemindTask extends TimerTask {
    ...
    public void run() {
    	if(usingWii){
	        thereminX = wiimote.axisX;
	        thereminY = wiimote.axisY;
	        if (thereminY < 0){
	        	thereminY = 0;
	        }
	        if (thereminX < 0){
	        ...
	        System.out.println("ThereminX: "+ thereminX + ".... ThereminY: "+ thereminY);
    	...
    	if(wiimote.button0){
    		...
    		filter.start();
    	}else if(wiimote.button2){
    		filter.A0.set(1);
    		filter.A1.set(0.5);
    		filter.B1.set(0.3);
    		System.out.println("Normal Pass");
    		filter.start();
    	}else if(wiimote.button3){
    		if(filter.B1.get() == -0.9){
        		filter.B1.set(0.3);
    		}
    		else{
    			filter.B1.set(-0.9);
    		...
    		filter.start();
    		System.out.println("Toggle Feedback");
    	}
...
public ThereminApp() {
	...
	c.gridwidth=3;
	add(logo, c);
   // GridLayout dave = new GridLayout(0,1);
...
/* Start synthesis engine.  */
	...
	scope.createProbe( mixer.output, "Output", Color.yellow );
	scope.finish();
	scope.hideControls();
	*/
...
public void changed(JXInputAxisEvent arg0) {
	//thereminX = wiimote.axis0;
	//thereminY = wiimote.axis1;
